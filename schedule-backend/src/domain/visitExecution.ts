...
export class VisitExecution extends AggregateRoot<VisitExecutionProps> {
  ...
  get dockId(): string { return this.props.dockId; }
  ...
  public static create(props: { vvnId: string; vesselId: string; dockId: string; arrivalTime: string; creatorId: string }, id?: UniqueEntityID): Result<VisitExecution> {
    const guardedProps = [
      { argument: props.vvnId, argumentName: 'vvnId' },
      { argument: props.vesselId, argumentName: 'vesselId' },
      { argument: props.vesselId, argumentName: 'dockId' },
  ...
  public updateBerthAndDock(props: { arrivalTime?: string; dockId?: string;}): Result<void> {
    if (props.arrivalTime !== undefined) {
      const arrival = new Date(props.arrivalTime);
      if (isNaN(arrival.getTime())) {
        return Result.fail<void>("Invalid arrivalTime.");
    ...
    if (props.dockId !== undefined) {
      if (props.dockId.trim().length === 0) {
        return Result.fail<void>("dockId cannot be empty.");
      }
      this.props.dockId = props.dockId;
    }
