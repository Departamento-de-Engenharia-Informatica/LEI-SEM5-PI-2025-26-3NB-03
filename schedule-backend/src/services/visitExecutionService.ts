...
export default class VisitExecutionService implements IVisitExecutionService {
  ...
  public async updateVisitExecution(id: string, dto: IUpdateVisitExecutionDTO): Promise<Result<IVisitExecutionDTO>> {
    try {
  ...
  public async updateBerthAndDock(id: string, dto: IUpdateBerthDockDTO): Promise<Result<IVisitExecutionDTO>> {
    ...
    const updateResult = visit.updateBerthAndDock(dto);
    ...
    const savedVisit = await this.visitExecutionRepo.save(visit);
    return Result.ok<IVisitExecutionDTO>(VisitExecutionMap.toDTO(savedVisit));
  }
}
